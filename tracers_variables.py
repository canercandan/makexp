...
class Dummy(Tracer):
    """
    Just a dummy tracer class to use if you don't want to use tracer with stat.
    """
...
class Easy(Tracer):
    ...
    def trace(self, options, tree):
        ...
        tree['FILENAME'] = '%(NAME)s_%(MANGLENAME)s' % tree

        if self.title: tree['TITLE'] = self.title.replace(' ', '_')
        elif tree['TITLE']: tree['TITLE'] = tree['TITLE'].replace(' ', '_')
        else: tree['TITLE'] = 'notitle'

        for tree['EXT'] in tree['EXTENSIONS']:
            fig.savefig('%(GRAPHDIR)s/%(TITLE)s_%(FILENAME)s.%(EXT)s' % tree, format=tree['EXT'], dpi=280)
